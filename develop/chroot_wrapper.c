...
int main(int argc, char** argv) {
	...
	if(strlen(argv[2]+6) > 100) {
		fprintf(pErrFileP, "Your path variable is too large, this might be an attempt to compromise the script\n");
		fprintf(pErrFileP, "Exiting program, please shorten your path variable (use symlinks if you need to)\n");
		fprintf(pErrFileC, "Your path variable is too large, this might be an attempt to compromise the script\n");
		fprintf(pErrFileC, "Exiting program, please shorten your path variable (use symlinks if you need to)\n");
		return -1;
	}
	...
	/*fprintf(pErrFile, "Entered the child loop\n");*/
		...
		if(result == -1) {
			char* msg;
			msg = (char*)strerror(errno);
			fprintf(pErrFileC, "Child: An error has occurred during the chroot call:\
				%d\n%s\n", errno, msg);
		...
		if(result == -1) {
			char* msg;
			msg = (char*)strerror(errno);
			fprintf(pErrFileC, "Child: An error has occurred during the chdir call:\
				%d\n%s\n", errno, msg);
		...
		if(result == -1) {
			char* msg;
			msg = (char*)strerror(errno);
			fprintf(pErrFileC, "Child: An error has occurred during the setresgid \
				call: %d\n%s\n", errno, msg);
		...
		if(result == -1) {
			char* msg;
			msg = (char*)strerror(errno);
			fprintf(pErrFileC, "Child: An error has occurred during the \
				setresuid call: %d\n%s\n", errno, msg);
			fprintf(pErrFileC, "This probably means that the process \
				was not privileged enough to make this call\n");
			return -1;
		}
